...
Controllers.getUserAccessKey = function (req, res, next) {
	log(req.session.userAccessKey || 'bulunmadi');
...
Controllers.setUserAccessKey = function (req, res, next) {
	...
	log('oluşturuldu: ' + req.session.userAccessKey + req.session.userName);
	res.send('oluşturuldu: ' + req.session.userAccessKey + req.session.userName);
};

Controllers.CheckSession = function (req, res, next) {
	log('check session request begin');
	if (req.session.userAccessKey && req.session.userAccessKey !== '') {
		log('userAccessKey found: ' + req.session.userAccessKey + ':');
		log('rendering check_session page');
		res.render('check_session');
	} else {
		log('userAccessKey Not Found');
		res.redirect('/');
	}
	log('check session request end');
};
//maybe need
//var app = express();
//app.use(express.bodyParser());
Controllers.AuthFromBaseSite = function (req, res, next) {
	log('checking session begin:' + req.session.userAccessKey + ':::');
	res.contentType('json');
	if (req.session.userAccessKey && req.session.userAccessKey !== '') {
		log('user access key info found: ' + req.session.userAccessKey + ':');
		log('calling api/getUserInfoById begin');
		ControlUserInfo(req, res);
	} else {
		log('user access key info not found. response is /register url redirect');
		res.status(200).send(prepareResult(false, '/register', 'local', false));
	}
	log('checking session end');
};
function log(msg) {
	console.error(msg);
	winston.log(msg);
...
function ControlUserInfo(req, res) {
	log('OnControlUserInfo');
	log(req.session.userAccessKey);
	...
	if (result.content && result.content !== '' && result.content.length > 5) {
		...
		if (takenUserData.result) {
			log('result controlling begin');
			Controllers.login(takenUserData, function (err, user) {
				if (!err) {
					log('no error. user will login');
					Authentication.doLogin(req, user.uid, function (err) {
						if (err) {
							log('logined failed error code X6 :' + err);
							res.status(200).send(prepareResult(false, '/register', 'local', false));
						} else {
							log('no error user logined sussecfully');
							res.status(200).send(prepareResult(true, '/', 'local', true));
						}
					});
				} else {
					log('error code X5 :' + err);
				}
			});
		} else {
			log('result == false redirecting to /register');
			res.status(200).send(prepareResult(false, '/register', 'local', false));
		}
	} else {
		log('result == false redirecting to /register');
...
Controllers.login = function (userData, callback) {
	...
	Controllers.getUidByOAuthid(payload.oAuthid, function (err, uid) {
		...
		if (uid !== null) {
			...
			log('This is New User ------------');
			// New User
			var success = function (uid) {
				// Save provider-specific information to the user
				log('on success begin uid:' + uid);
				User.setUserField(uid, payload.oAuthType + 'Id', payload.oAuthid);
				log('setUserField: ' + payload.oAuthType + 'Id    oAuthid:' + payload.oAuthid);
				db.setObjectField(payload.oAuthType + 'Id:uid', payload.oAuthid, uid);
				log('setObjectField: ' + payload.oAuthType + 'Id:uid');
				if (payload.isAdmin) {
					log('this is admin.');
			...
			User.getUidByEmail(payload.email, function (err, uid) {
				...
				if (!uid) {
					log('İts really new user. will create a new account');
					log(payload.handle || 'tempNick');
					User.create({
						...
						log('new account created step1');
						success(uid);
					});


				} else {
					log('user already registerd with this email :' + payload.email + ': we will merging accounts');
